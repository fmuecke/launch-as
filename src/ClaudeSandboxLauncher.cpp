...
{
  public:
    explicit SecretBuffer(std::size_t capacityCharacters) : characters_(capacityCharacters, L'\0')
    {
    }
    ...
    [[nodiscard]] wchar_t* data() noexcept { return characters_.data(); }

    [[nodiscard]] const wchar_t* data() const noexcept { return characters_.data(); }

    [[nodiscard]] std::size_t capacity() const noexcept { return characters_.size(); }
...
[[nodiscard]] DWORD LoadStoredPassword(const AccountIdentity& account, SecretBuffer& password)
{
    CredentialBuffer credential;
    const CredentialReadResult result = ReadCredential(account, credential);
    if (result == CredentialReadResult::NotFound)
...
[[nodiscard]] DWORD Run(const AccountIdentity& account, const Options& options)
{
    SecretBuffer password(MaximumPasswordCharacters);
    ...
    std::wstring commandLine = sandbox_launcher::BuildWindowsCommandLine(
        options.executablePath.native(), options.processArguments);
