...
int wmain()
{
    ...
    launch_as::test::TemporaryDirectory credentialDirectory;
    ...
    launch_as::broker::RegistrationService registration(credentialDirectory.path().native());
    account.MarkCreated();
    ...
    const DWORD brokerTokenError =
        resetError == ERROR_SUCCESS
            ...
            : resetError;
    password.Clear();
    launch_as::broker::EnrollmentStore enrollments(credentialDirectory.path().native());
    ...
    launch_as::broker::RegistrationService privilegedRegistration(
        credentialDirectory.path().native());
