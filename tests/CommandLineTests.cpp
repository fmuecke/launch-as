...
int wmain(int argc, wchar_t* argv[]) {
    ...
    if (!ExpectEqual(
            sandbox_launcher::QuoteWindowsCommandLineArgument(L""),
            L"\"\"",
            L"empty argument") ||
        !ExpectEqual(
            sandbox_launcher::QuoteWindowsCommandLineArgument(L"plain"),
            L"plain",
            L"plain argument") ||
        !ExpectEqual(
            sandbox_launcher::QuoteWindowsCommandLineArgument(L"with space"),
            L"\"with space\"",
            L"spaced argument") ||
        !ExpectEqual(
            sandbox_launcher::QuoteWindowsCommandLineArgument(
                L"quote\"inside"),
            L"\"quote\\\"inside\"",
            L"embedded quote") ||
        !ExpectEqual(
    ...
    if (!CreateProcessW(
            ...
            &startupInfo,
            &processInfo)) {
        std::wcerr << L"Could not start argv probe: "
                   << GetLastError() << L"\n";
        return 1;
    }
    ...
    if (waitResult != WAIT_OBJECT_0) {
        std::wcerr << L"Could not wait for argv probe: "
                   << GetLastError() << L"\n";
        return 1;
    }
    ...
    if (!GetExitCodeProcess(process.get(), &exitCode)) {
        std::wcerr << L"Could not read argv probe exit code: "
                   << GetLastError() << L"\n";
        return 1;
    }
    if (exitCode != 0) {
